 ...
 * @version 1.8.0
 */
public class SpotbugTool extends Tool {
   ...
   */
  public SpotbugTool(File inputAnalysis) {
   ...
   */
  public SpotbugTool(File inputAnalysis, Report inputReport) {
    analysis = inputAnalysis;
    report = inputReport;
  }
  ...
  public File analyze(ArrayList<File> files) {
    ...
    System.out.println("Running SpotBugs on selected Class");
    ...
    if (NUMBER_OF_FILES >= 1) {
      try {
		...
		*/
		...
		command = "/SPOTBUGS/bin/spotbugs -textui -jvmArgs -debug -medium " + sourceDirectoryName + "";

        if (SystemUtils.isWindows()) {
            command = "C:\\SPOTBUGS\\bin\\spotbugs.bat -textui -jvmArgs -debug -medium " + sourceDirectoryName + "";
        }
        ...
        e.printStackTrace();
        System.out.println("Couldn't run Spotbugs: " + e.getMessage());
    ...
    System.out.println("SpotBugs run completed");
    
    if (outputFileName != null) {
      System.out.println(outputFileName);
      outputFileName = outputFileName + File.separator + "spotbug-analysis.xml";
